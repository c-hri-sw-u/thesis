...
- **4.2 Capture Layer** — Vision + Audio + IMU roles, dual role distinction (trigger vs. data source), hybrid capture reasoning
- **4.3 Preprocessing** — Why preprocess (token cost, info density), batch boundary design, frame filtering
- **4.4 Inference** — Why batch over per-frame (design argument to exclude Path A), VLM prompt design, output format
- **4.5 Memory Integration** — Hybrid strategy (silent session + perception log), why leverage OpenClaw's existing pipeline, nightly summarization
...
- **Experiment variables (marked 🧪 in design-space.md)** are the subset of decisions explored during the study: capture interval, prompt wording, batch window size, nightly summarization on/off, proactive cron phased addition. These produce findings for RQ3.
