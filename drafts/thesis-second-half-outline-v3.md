...
| 阶段 | 时间 | 系统状态 | 观察方式 | 目的 |
|------|------|---------|---------|------|
| Phase 0: Calibration | Pre-study (~1 week) | Active iteration（调 adaptive interval、SSIM threshold、prompt、batch window、importance weights、cron frequency） | 观察 perception log 质量，识别故障模式 | 稳定 pipeline；resolve open design variables（见 Chapter 5 Table 2） |
| Phase 1: Observation | 2 weeks | 所有参数 frozen | 被动观察 + structured probes + proactive cron（三种方式并行） | 收集 RQ1、RQ2、RQ3 的数据 |
...
Phase 1 同时使用三种方式。Proactive cron 从 Phase 1 开始即参与（频率在 Phase 0 中确定），不再分阶段引入。

**迭代作为研究活动：**

Phase 0 中的所有迭代（prompt 修改、参数调整、pipeline 修复）都被完整记录，包括：改了什么、为什么改、改前改后的输出对比。这些迭代记录是 RQ3 的直接数据来源——design requirements 从中提炼。

Phase 1 中，pipeline 配置保持 frozen。如果出现必须修改的问题，记录为 finding 但不改动，除非问题导致 pipeline 完全无法运行。
...
| OpenClaw three-tier memory output | 三层 memory 的实际内容（physical-logs / physical-insights / physical-pattern.md 的变化，以及 OpenClaw 原生 memory 的变化） | RQ3：三层存储中信息的流转、保留与丢弃 |
| 交互记录 | 研究期间所有与 OpenClaw 的对话 transcript（含 structured probe sessions 的标注） | RQ2：哪些交互中物理信息有用 / 无用 |
| Proactive messages log | Phase 1 中 cron job 触发的所有 agent 主动消息（含 agent 决定不行动的记录） | RQ2：proactive surfacing 的质量 |
...
- **三层 memory 流转分析：** physical-logs → physical-insights → physical-pattern.md 的信息损失和提炼质量；OpenClaw retrieval 实际 surface 了哪一层的内容 → RQ3
- **日记 + 迭代记录的主题分析：** 从反思和迭代决策中提取 recurring themes → design implications
- **三种观察方式横向对比：** 被动观察 / structured probes / proactive cron 各自揭示的物理信息利用率和质量
...
**三种观察方式的交叉对比**（同一阶段内三种 lens 的互补发现）

### 6.3 Pipeline Behavior and Design Iterations（→ RQ3）
- Phase 0 中的迭代记录：改了什么、为什么、效果如何
- 三层 memory（physical-logs / physical-insights / physical-pattern.md）的信息流转分析：哪些信息在 distillation 中保留、丢弃、被错误提炼
- 具体的失败案例（pipeline 产出低质量 memory、nightly job 丢弃有价值信息或提炼出错误 pattern、nightly summary 的效果等）
- Retrieval & context loading 的实际表现：physical-pattern.md 的 bootstrap injection 是否让 agent 自然利用持久 pattern；physical-insights 的 session-start read 效果；physical-logs 的 on-demand retrieval 命中率
...
- **Three-tier memory architecture：** 三层存储（full-volume logs / daily distillation / persistent patterns）是否是合理的 granularity 切分；nightly summarization 作为中间层的必要性
- **Retrieval visibility design：** bootstrap injection / session-start read / on-demand search 三层 visibility 对 agent 行为的影响；哪一层最关键
- **Adaptive capture vs. fixed interval：** trigger-driven 动态密度在实际佩戴中的表现；max_interval 选择对 pattern accumulation 的影响
