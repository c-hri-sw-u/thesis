...
| **006** | MARC: Memory-Augmented Reinforcement Learning for Conversational Agents | 2024 | Wu et al. | 2.3 | Reinforcement learning with episodic memory for conversational agents. Uses memory to improve dialogue state tracking and response generation. Relevance: Shows RL+memory pattern, but focused on dialogue not multimodal egocentric data. | > "We propose MARC, a memory-augmented reinforcement learning framework that enables conversational agents to learn from episodic memory through policy optimization." (Abstract) |
